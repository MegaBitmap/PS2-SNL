...
namespace SNL_CLI
{
    internal class FTP
    {
        ...
        public static void UploadFile(FtpClient client, string filePath, string ftpPath, string targetFile)
        {
            ...
            {
                Console.WriteLine($"Failed to upload file {filePath} to the PS2 via FTP.\n{ex.Message}\n{ex.InnerException}\nPlease make sure there is 1.5MB of unused free space on the memory card or USB drive.");
                client.Disconnect();
                MiscMethods.PauseExit(14);
            }
        }
